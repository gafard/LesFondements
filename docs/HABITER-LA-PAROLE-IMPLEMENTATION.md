...
La construction distante dispose de la configuration cliente publique dans `config/client-public.json` (Firebase, adresse du site et clé publique de notification), surchargeable par l’environnement. Les clés privées des services restent dans les variables secrètes de l’hébergement.
